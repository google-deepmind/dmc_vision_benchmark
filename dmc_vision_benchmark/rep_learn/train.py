...
from dmc_vision_benchmark.data import env_dmc_vb
...
class Trainer:
  ...
  def __init__(self, config: ml_collections.ConfigDict):
    ...
    if self.config.model in ["td3_bc", "td3_bc_on_state"]:
      self.config.obs_preproc.nsteps_idm = 1
      self.config.data.add_rewards = True
    elif self.config.model == "pretrain_fwd_model":
      self.config.obs_preproc.nsteps_idm = 1
      self.config.data.add_rewards = False
    elif self.config.model == "pretrain_idm":
      self.config.data.add_rewards = False
    else:
    ...
    # Load the data
    assert self.config.dataset == "dmc_vb"
    ...
    self.checkpoint_workdir = (
        f"{self.config.save_to_folder}/checkpoint/{this_id}"
    )
    self.online_workdir = (
        f"{self.config.save_to_folder}/online_eval/{this_id}"
    )
    ...
    self.online_env = eval_in_envs.EvalInEnv(
        ...
        env_eval_seed_tuples=[
            (x, 0)
            for x in range(
                int(1e6), int(1e6) + self.config.online_eval.num_online_runs
            )
        ],
        render=True,
  ...
  def _build_model(self) -> Any:
    ...
    # Encoder modules
    shared_obs_encoder_default = nn.Sequential([
        # Stack inputs first
    ...
    shared_state_encoder_default = lambda x: einops.rearrange(
        x, "... n_frames d -> ... (n_frames d)"
    )
    actor_encoder_trunk_default = nn.Sequential([  # same as MLPMultiHeads
        nn.Dense(
            features=self.config.encode_dim,
            name="trunk",
    ...
    critic_encoder_trunk_default = nn.Sequential([   # same as MLPMultiHeads
        nn.Dense(
            features=self.config.encode_dim,
            name="trunk",
            kernel_init=nn.initializers.orthogonal(),
    ...
    # Encoder modules
    obs_encoder_default = nn.Sequential([
        # Stack inputs first
        lambda x: einops.rearrange(
            x, "... n_frames h1 w1 c1 -> ... h1 w1 (n_frames c1)"
        ),
    ...
    state_encoder_default = nn.Sequential([
    ...
    next_action_predictor_default = mlp.MLP(
        features=(256, 256),
        out_dim=self.action_dim,
        activation=nn.relu,
        out_activation=nn.tanh,  # Since actions are between -1 and 1
        name="next_action_predictor",
        kernel_init=nn.initializers.orthogonal(),
    )
    ...
    inverse_dynamics_predictor_default = mlp.MLP(
        features=(256, 256),
        out_dim=self.action_dim,
        activation=nn.relu,
        out_activation=nn.tanh,  # Since actions are between -1 and 1
        name="inverse_dynamics_predictor",
        kernel_init=nn.initializers.orthogonal(),
    )
    state_predictor_default = nn.Dense(
        features=self.gt_state_dim,
        kernel_init=nn.initializers.orthogonal(),
        name="state_predictor",
    )
    action_encoder_default = mlp.MLP(
        features=(256, 256),
        out_dim=self.config.encode_dim,
        activation=nn.relu,
        out_activation=nn.tanh,  # Same as obs_encoder
        name="action_encoder",
        kernel_init=nn.initializers.orthogonal(),
    )
    forward_model_default = mlp.MLP(
        features=(256, 256),
        out_dim=self.config.encode_dim,
        activation=nn.relu,
        out_activation=nn.tanh,  # Same as obs_encoder
    ...
    def _build_td3_bc(shared_obs_encoder, pass_state):
      """Builds the TD3+BC agent."""
      return td3_bc.TD3BC(
          encode_dim=self.config.encode_dim,
          actions_dim=self.action_dim,
          shared_obs_encoder=shared_obs_encoder,
          actor_encoder_trunk=actor_encoder_trunk_default,
          critic_encoder_trunk=critic_encoder_trunk_default,
          actor=actor_default,
          critic1=critic1_default,
          critic2=critic2_default,
          # TD3 + BC params
    ...
    def _build_mlp(
        obs_encoder=obs_encoder_default,
    ...
    if self.config.model == "td3_bc":
      return _build_td3_bc(shared_obs_encoder_default, pass_state=False)

    if self.config.model == "td3_bc_on_state":
      return _build_td3_bc(
          shared_state_encoder_default,
          pass_state=True,
      )

    if self.config.model == "bc_on_state":
      return _build_mlp(
          obs_encoder=state_encoder_default,
          next_action_predictor=next_action_predictor_default,
          pass_state=True,
      )
    ...
    if self.config.model == "pretrain_idm":
      return _build_mlp(
          inverse_dynamics_predictor=inverse_dynamics_predictor_default,
          idm_step_encoder=idm_step_encoder_default,
      )

    if self.config.model == "pretrain_fwd_model":
      return _build_mlp(
          forward_model=forward_model_default,
          action_encoder=action_encoder_default,
      )
    ...
    if self.config.model == "bc_trained_w_state":
      return _build_mlp(
          next_action_predictor=next_action_predictor_default,
          state_predictor=state_predictor_default,
      )
  ...
  def _update_mlp(
    ...
    Returns:
      losses: Loss on the batch
      new_state: New trainer state
    """
  ...
  def loss_on_batch(
    ...
    Returns:
      loss: Loss
    """
    assert self.config.model != "td3_bc"
    ...
    if self.config.model in [
      ...
      bc_loss = self._square_l2_dist(
          res["pred_actions"], batch_data["actions"][:, 0]
      )
      avg_loss += bc_loss

    if self.config.model in ["pretrain_idm"]:
      idm_loss = self._square_l2_dist(
          res["inv_model_actions"], batch_data["actions"][:, 0]
      )
      avg_loss += idm_loss

    if self.config.model in ["bc_trained_w_state"]:
      state_pred_loss = self._square_l2_dist(
          res["pred_states"], batch_data["states"][:, 0]
      )
      avg_loss += state_pred_loss

    if self.config.model in ["pretrain_fwd_model"]:
  ...
  def loss_on_batch_td3_bc(
    ...
    """Computes the loss of the TD3 + BC network on a given batch.
    ...
    Returns:
      loss: Loss
    """
    assert self.config.model in ["td3_bc", "td3_bc_on_state"]
    ...
    if loss_type == "critic":
      ...
      critic_loss = self._square_l2_dist(
          res_critics["q1"], res_critic_target["target_q"]
      ) + self._square_l2_dist(res_critics["q2"], res_critic_target["target_q"])
      return critic_loss
  ...
  def load_model_params(
      self, pretrained_model_id: int
  ) -> Dict[str, jnp.ndarray]:
    """Load the model params from a pretrained model."""
    load_checkpoint_workdir = (
        f"{self.config.save_to_folder}/checkpoint/{self.config.pretrained_model_id}"
    )
  ...
  def train(self):
    ...
    if self.config.model not in ["td3_bc", "td3_bc_on_state"]:
      future_obs = (
    ...
    teacher_params = None
    if self.config.model in ["pretrain_fwd_model", "td3_bc", "td3_bc_on_state"]:
    ...
    # Option1: Optionally restore checkpoint
    if last_step is not None:
      print("Restoring checkpoint from step", last_step)
    ...
    # Option 2: Initialize the state encoder from a pretrained model
    elif self.config.model in ["bc_w_frozen_encoder"]:
      print("Loading pretrained model", self.config.pretrained_model_id)
      model_params_loaded = self.load_model_params(
      ...
      print("No checkpoint found at", self.checkpoint_workdir)
    ...
    for it in range(num_iters):
      ...
      if (
          it + 1
      ) % self.config.learning.eval_every == 0 and self.config.model not in [
          "td3_bc",
          "td3_bc_on_state",
      ]:
        # Evaluate
      ...
      if it % self.config.learning.online_eval_every == 0:
        if self.config.model not in ["pretrain_idm", "pretrain_fwd_model"]:
